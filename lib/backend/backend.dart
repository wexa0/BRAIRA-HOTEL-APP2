...
Future maybeCreateUser(User user) async {
  final userRecord = GuestRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await GuestRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createGuestRecordData(
    email: user.email ??
  ...
  currentUserDocument = GuestRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createGuestRecordData(email: email));
}
